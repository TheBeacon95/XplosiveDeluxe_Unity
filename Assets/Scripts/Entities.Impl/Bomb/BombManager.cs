...
public class BombManager : MonoBehaviour, BombManagerIfc {
    ...
    /// </returns>
    public UnityEvent<Vector2Int> CreateBomb(BombProperties properties, Vector2Int position) {
        if (!m_levelManager.IsBombPlaceable(position)) {
            return null;
        }
        else {
            // Todo: add properties.
            Bomb bomb = Instantiate(m_bombPrefab, new Vector3(position.x, position.y), Quaternion.identity, transform);
            bomb.Properties = properties;
            m_activeBombs.Add(position);
            bomb.DestroyedEvent.AddListener(OnBombDestroyed);
            m_explosionManager.Add(bomb);
    ...
    private void Awake() {
        ...
        m_explosionManager = (ExplosionManager)m_levelManager.GetManager<ExplosionManagerIfc>();
    }
