...
public class PlayerController : MonoBehaviour, PlayerIfc {
    ...
    [SerializeField]
    private MonoBehaviour m_levelManagerMono;
    ...
    private void Awake() {
        // Todo: solve
        //m_levelManager = FindObjectOfType<MonoBehaviour>().OfType<LevelManagerIfc>();
        if (m_levelManagerMono is LevelManagerIfc) {
            m_levelManager = (LevelManagerIfc)m_levelManagerMono;
        }
    }
    ...
    private void PlaceBomb() {
        ...
        properties.Range = m_bombLength;
        properties.IsBrickMaker = m_isBombCrazy;
    ...
    private bool IsBombAvailable() {
        return (m_bombCount - m_activeBombs) > 0;
    }
